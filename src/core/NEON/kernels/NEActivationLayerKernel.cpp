...
namespace
{
	Status validate_arguments(const ITensorInfo *input, const ITensorInfo *output, const ActivationLayerInfo &activation_info)
	{
		    ARM_COMPUTE_RETURN_ERROR_ON_CPU_F16_UNSUPPORTED(input);
		        ARM_COMPUTE_RETURN_ERROR_ON_DATA_TYPE_CHANNEL_NOT_IN(input, 1, DataType::S8 , DataType::U8, DataType::QASYMM8, DataType::QSYMM16, DataType::F16, DataType::F32);
			    ...
			    if(output != nullptr)
				    ...
				    static std::map<ActivationFunction, ActivationFunctionExecutorPtr> act_map_f32 =
					    ...
					    static std::map<ActivationFunction, ActivationFunctionExecutorPtr> act_map_qasymm8 =
						    ...
						    static std::map<ActivationFunction, ActivationFunctionExecutorPtr> act_map_s8 =
							        {
									        { ActivationFunction::RELU, &NEActivationLayerKernel::activation<ActivationFunction::RELU, int8_t> },
										    };
																            ...
																            break;
																	            case DataType::F32:
																	                _func = act_map_f32[activation_info.activation()];
																			            break;
																				                case DataType::S8:
																				                _func = act_map_s8[activation_info.activation()];
																						            break;
#ifdef __ARM_FEATURE_FP16_VECTOR_ARITHMETIC
																							            case DataType::F16:
...
typename std::enable_if<std::is_same<T, int8_t>::value, void>::type NEActivationLayerKernel::activation(const Window &window)
{
		 ...
		 *     const auto               window_end_x   = static_cast<int>(window.x().end());*/
		    ...
		    const auto               window_end_x   = static_cast<int>(_input->info()->dimension(0));
		        const ActivationFunction act            = F;

			    /*Window win_collapsed = window.collapse_if_possible(window, Window::DimZ);*/
			    Window win_collapsed = window;
			        win_collapsed.set(Window::DimX, Window::Dimension(0, 1, 1));
				    win_collapsed.set(Window::DimY, Window::Dimension(0, _input->info()->dimension(1), 1));
				        win_collapsed.set(Window::DimZ, Window::Dimension(0, _input->info()->dimension(2), 1));
					    ...
					    Iterator input(_input, win_collapsed);
						    ...
						    int8x16_t const_0=vdupq_n_s8(0);
							    ...
							    execute_window_loop(win_collapsed, [&](const Coordinates &id)
									        {
										        size_t x_dimension=id[0], y_dimension=id[1], z_dimension=id[2];
											       
											        const int8_t* input_ptr  = reinterpret_cast<const int8_t*>(_input->buffer()+_input->info()->offset_element_in_bytes(Coordinates(x_dimension, y_dimension, z_dimension)));
												        int8_t* output_ptr = reinterpret_cast<int8_t*>(_output->buffer()+_output->info()->offset_element_in_bytes(Coordinates(x_dimension, y_dimension, z_dimension)));
																			             ...
																			             const int8_t in = *(reinterpret_cast<const int8_t *>(input_ptr + x));
																				                 int8_t       tmp;
																						             switch(act)
																								                 {
																											                 case ActivationFunction::RELU:
																														                     tmp = std::max<int8_t>(static_cast<int8_t>(0), in);
																																                         break;
																																			                 default:
																																			                     ARM_COMPUTE_ERROR("Unsupported activation function");
																																					                 }
